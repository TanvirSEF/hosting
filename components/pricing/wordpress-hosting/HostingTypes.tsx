...
import { Sparkles, Zap, Rocket, LucideIcon } from 'lucide-react';
...
const DEFAULT_WORDPRESS_TYPES: PlanType[] = [
  {
    key: 'starter',
    iconKey: 'starter',
...
export default function HostingTypes() {
  const t = useTranslations('wordpressHosting.hostingTypes');
  const planMetrics = (() => {
    try {
      const raw = t.raw('planMetrics') as unknown;
      return Array.isArray(raw) ? (raw as PlanType[]) : DEFAULT_WORDPRESS_TYPES;
    } catch {
      return DEFAULT_WORDPRESS_TYPES;
    }
  })();

  const getIconForPlan = (iconKey?: string): LucideIcon => {
    if (iconKey === 'business') return Zap;
    ...
    <section className="relative overflow-hidden bg-[#FAFAFA] py-16 md:py-24">
      ...
      <div className="relative z-10 container mx-auto px-4">
        ...
        <div className="mt-12 text-center md:mt-16">
          ...
          >
            <Link href="/wordpress-hosting#pricing">
              {t('getStarted')}
            </Link>
          </Button>
        </div>
