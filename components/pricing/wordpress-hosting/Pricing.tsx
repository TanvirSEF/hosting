...
export default function Pricing() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const plansResult = await getWordPressPlansAction(currency, locale);
...
function PricingContent({ plans, discountRules }: { plans: any[] | null, discountRules: Record<number, number> }) {
  const t = useTranslations('wordpressHosting.pricing');
  const { currency, formatPrice } = useCurrency();
        ...
        {plans.map((plan: any, index: number) => {
          ...
          return (
            <div
              key={plan.id || index}
              className={cn(
                'erfan-card group relative flex flex-col rounded-[20px] border bg-white p-6 sm:p-8 transition-all duration-300',
                plan.highlight
            ...
            >
              ...
              <div className="mb-2 flex flex-col gap-1">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="font-roboto text-xl font-medium text-[#1E1F21]"><span className="text-base font-normal text-[#667085] ml-1">
                      /{plan.unit.replace('/', '')}
                    </span>
                  </span>
                </div>
