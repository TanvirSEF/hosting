...
export default function HostingTypes() {
  const t = useTranslations('webHosting.hostingTypes');
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        const result = await getSharedHostingPlansForHomeAction(undefined, locale);
  ...
  return (
    <section className="relative overflow-hidden bg-[#FAFAFA] py-16 md:py-24">
      ...
      <div className="relative z-10 container mx-auto px-4">
        ...
        <div className="mx-auto w-full max-w-[1360px] relative px-4 sm:px-12">
          <Carousel
            opts={{
              align: 'start',
              loop: false,
            ...
            className="w-full"
          >
            <CarouselContent className="md:-ml-4 xl:-ml-6 py-4">
              {plans.map((plan, idx) => {
            ...
            const websiteFeature =
              plan.features.find((f: string) => /website|site/i.test(f)) ||
            ...
            const websiteCount = websiteFeature.match(/\d+/)?.[0] || websiteFeature;
            const storageAmount = storageFeature.match(/\d+/)?.[0] || storageFeature;
            // Simplified mocks for visual bars if not explicit in API
                const performanceLabel = getPerformanceLabel(plan.performanceLevel);

                return (
                  <CarouselItem key={idx} className="pl-4 md:pl-8 xl:pl-6 md:basis-1/2 lg:basis-1/3">
                    <div
                      className="group relative flex min-h-[400px] flex-col h-full rounded-[24px] border border-transparent bg-white p-6 transition-shadow duration-300 hover:border-purple-100 hover:shadow-xl md:rounded-[32px] md:p-8"
                    >
                      <div className="mb-8">
            ...
            <div className="hidden sm:block">
              <CarouselPrevious className="-left-4 sm:-left-12 xl:-left-14" />
              <CarouselNext className="-right-4 sm:-right-12 xl:-right-14" />
            </div>
          </Carousel>
        </div>
        ...
        <div className="mt-12 text-center md:mt-16">
          ...
          >
            <Link href="/shared-hosting#pricing">
              {t('getStarted')}
            </Link>
          </Button>
        </div>
