...
export default function Comparison() {
  const t = useTranslations('webHosting.comparison');
  ...
  const [activePlanIdx, setActivePlanIdx] = useState(1); // Default to center/middle
    ...
    const fetchData = async () => {
      ...
      try {
        const [plansResult, discountsResult] = await Promise.all([
          getSharedHostingPlansForHomeAction(currency, locale),
  ...
  // Scroll-based visibility check for sticky footer
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container mx-auto max-w-7xl px-4 md:px-8">
        ...
        {/* Desktop Sticky Header - Clean without boxes */}
        <div className="sticky top-[80px] z-40 mb-8 hidden grid-cols-5 gap-4 lg:gap-8 border-b border-gray-200/60 bg-[#FAFAFA]/95 py-6 backdrop-blur-md lg:grid">
          <div className="col-span-1" />
          {processedPlans.slice(0, 4).map((plan, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <div className="mb-3 flex flex-col items-center">
                {plan.hasDiscount && (
                  <span className="text-[#AEAEAE] text-xs font-medium line-through decoration-slate-400/80 mb-0.5">
        ...
        {/* Mobile Sticky Tabs - No scrollbar, no boxes, pill style */}
        ...
        <div className="space-y-6 pb-32 lg:pb-0">
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="space-y-0">
              {t.raw('topFeaturesList')?.map((row: any, rIdx: number) => (
                <div key={rIdx}>
                  {/* Desktop Row */}
                  <div className="hidden grid-cols-5 items-center gap-4 border-b border-gray-100 py-4 transition-colors last:border-0 hover:bg-gray-50/50 lg:grid">
                    <div className="col-span-1">
                      <span className="cursor-help border-b border-dotted border-gray-300 text-[15px] text-gray-700">
                        {row.feature}
                      </span>
                    </div>
                    {processedPlans.slice(0, 4).map((plan, pIdx) => {
                      const val = pIdx === 0 ? row.plan1 : pIdx === 1 ? row.plan2 : pIdx === 2 ? row.plan3 : row.plan4;
          ...
          <Accordion type="multiple" className="w-full space-y-4">
            {[
              { id: 'security', title: t('security'), features: t.raw('securityFeatures') as { feature: string }[] },
              { id: 'service', title: t('support'), features: t.raw('supportFeatures') as { feature: string }[] },
            ].map((section) => (
              <AccordionItem
                key={section.id}
                value={section.id}
              ...
              >
                ...
                <AccordionContent className="pt-2 pb-6">
                  {section.features?.map((item: any, rIdx) => (
                    <div key={rIdx}>
                      <div className="hidden grid-cols-5 items-center gap-4 border-b border-gray-100 py-4 last:border-0 lg:grid">
                        <div className="col-span-1">
                          <span className="text-[15px] text-gray-700">
                            {item.feature}
                          </span>
                        </div>
                        {processedPlans.slice(0, 4).map((_, pIdx) => {
                          const val = pIdx === 0 ? item.plan1 : pIdx === 1 ? item.plan2 : pIdx === 2 ? item.plan3 : item.plan4;
                          const displayVal = val !== undefined ? val : 'true';
                          return (
                            <div
                              key={pIdx}
      ...
      {/* Mobile Sticky Footer - Only visible when Comparison section is in view */}
      {activePlan && (
        <div
        ...
        >
          <div className="flex flex-col items-center gap-2">
            ...
            <Button onClick={() => handleAddToCart(activePlan)} className="h-12 w-full rounded-full bg-[#8C52FF] font-bold text-white shadow-lg shadow-[#8C52FF]/20 hover:bg-[#7839EE]">
              {t('choose')} {activePlan.name}
            </Button>
