...
import { Box, Gamepad2, Server, Database } from 'lucide-react';
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        'rounded-[20px] shadow-sm opacity-100',
        'transform-gpu'
      )}
      transition={{
        duration: 0.3,
...
function getIcon(index: number) {
  const icons = [Box, Gamepad2, Server, Database];
  const Icon = icons[index % icons.length];
  return <Icon className="h-6 w-6" />;
}

export default function VPSUseCases() {
  const t = useTranslations('vps.useCases');
  ...
  return (
    ...
    >
      ...
      >
        {/* Optimized background blob with simple opacity and GPU hint */}
        <div className="pointer-events-none absolute top-[5%] -right-[150px] z-0 h-[300px] w-[300px] rounded-full bg-[rgba(167,120,250,0.4)] blur-[100px] will-change-transform transform-gpu lg:top-[12%] lg:-right-[200px] lg:h-[650px] lg:w-[600px]" />
