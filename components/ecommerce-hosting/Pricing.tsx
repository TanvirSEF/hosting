...
import { getEcommercePlansAction } from '@/actions/hosting-actions';
...
export default function Pricing() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const plansResult = await getEcommercePlansAction(currency, locale);
...
function PricingContent({ plans, discountRules }: { plans: any[] | null, discountRules: Record<number, number> }) {
  const t = useTranslations('ecommerce-hosting.pricing');
  const { currency, formatPrice } = useCurrency();
